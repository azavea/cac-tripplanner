...
class EventTests(TestCase):
    def setUp(self):
        ...
        with open("default_media/square/BartramsGarden.jpg") as image_file:
            ...
            common_args = dict(
                description="Sample event for tests", image=test_image, wide_image=test_image
            )
...
class EventMultiDestinationTests(TestCase):
    def setUp(self):
        ...
        with open("default_media/square/BartramsGarden.jpg") as image_file:
            ...
            self.place_1 = Destination.objects.create(name="place_one", published=True, **dest_args)

            self.place_2 = Destination.objects.create(
                name="place_two", published=False, **dest_args
            )
    ...
    def test_event_destination_order(self):
        ...
        self.assertEqual(event_1_first_dest.destination.id, self.place_2.id)
...
class DestinationTests(TestCase):
    def setUp(self):
        ...
        with open("default_media/square/BartramsGarden.jpg") as image_file:
            ...
            self.place_1 = Destination.objects.create(
                name="place_one", published=True, **common_args
            )

            self.place_2 = Destination.objects.create(
                name="place_two", published=True, **common_args
            )

            self.place_3 = Destination.objects.create(
                name="place_three", published=False, **common_args
            )
    ...
    def test_place_detail_view(self):
        ...
        url = reverse("place-detail", kwargs={"pk": self.place_1.pk})
        response = self.client.get(url)
        self.assertContains(response, "place_one", status_code=200)

        # unpublished place should also be viewable
        url = reverse("place-detail", kwargs={"pk": self.place_3.pk})
        response = self.client.get(url)
...
class TourTests(TestCase):
    def setUp(self):
        ...
        tour_args = dict(description="Sample tour for tests")
        self.tour_1 = Tour.objects.create(name="tour_one", published=True, **tour_args)

        self.tour_2 = Tour.objects.create(name="tour_two", published=False, **tour_args)

        self.tour_1.tour_destinations.add(
            TourDestination.objects.create(
                destination=self.place_1, related_tour=self.tour_1, order=2
            )
        )

        self.tour_1.tour_destinations.add(
            TourDestination.objects.create(
                destination=self.place_2, related_tour=self.tour_1, order=1
            )
        )
        ...
        self.tour_2.tour_destinations.add(
            TourDestination.objects.create(
                destination=self.place_1, related_tour=self.tour_2, order=1
            )
        )

        self.tour_2.tour_destinations.add(
            TourDestination.objects.create(
                destination=self.place_2, related_tour=self.tour_2, order=2
            )
        )
    ...
    def test_tour_destination_order(self):
        ...
        self.assertEqual(tour_1_first_dest.destination.id, self.place_2.id)
        # other place has second order
